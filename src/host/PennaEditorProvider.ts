...
import {
  ...
  extError,
  extResponse,
} from "../webview/PennaBridge";

/** Host → Webview 的编辑器启动/重建载荷 */
export interface PennaBootPayload {
  text: string;
...
/**
 * Penna Markdown 的自定义编辑器提供程序。
 * 负责管理 Webview 的生命周期、文档同步以及与前端插件的 IPC 通信。
 */
export class PennaEditorProvider implements vscode.CustomTextEditorProvider {
  public static readonly viewType = "penna-markdown.editor";
  ...
  async resolveCustomTextEditor(
    ...
    const buildBoot = (): PennaBootPayload => ({
    ...
    vscode.workspace.onDidChangeConfiguration(
      (event) => {
        if (!event.affectsConfiguration("pennaMarkdown", document.uri)) {
          return;
        }
        config = new PennaConfig(document.uri);
        webviewPanel.webview.postMessage({
          command: "reconfigure",
          data: buildBoot(),
        } satisfies ExtMessage);
    ...
    webviewPanel.webview.onDidReceiveMessage(
      async (raw: ExtMessage) => {
        ...
        switch (raw.command) {
          ...
          case "uploadFile":
            try {
              const uploader = new PennaUploader(document.uri, config);
...
<head>
  ...
  <style>
    ...
    #penna-root { width: 100%; height: 100%; }
  </style>
</head>
<body>
  <div id="penna-root"></div>
